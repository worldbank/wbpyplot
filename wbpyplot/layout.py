...
def render_title_subtitle_note(fig, title, subtitle, note, wb_font_sizes, wb_spacing):
    ...
    if title:
        title_text = fig.text(margin_x_frac, y_pos, wrap_text(title, 80),
                              fontsize=wb_font_sizes['l'], fontweight='bold', color='#111111',
    ...
    if subtitle:
        subtitle_text = fig.text(margin_x_frac, y_pos, wrap_text(subtitle, 100),
                                 fontsize=wb_font_sizes['m'], fontweight='normal', color='#666666',
                                 ha='left', va='top', linespacing=1.2)
        fig.canvas.draw()
    ...
    for label, text in notes_to_render:
        ...
        label_artist = fig.text(x_start, y_note, label + " ",
                                fontsize=wb_font_sizes['s'], fontweight='bold', color='#111111',
                                ha='left', va='bottom')
        fig.canvas.draw()
        ...
        note_artist = fig.text(x_start + label_width_frac, y_note, wrap_text(text, 120),
                               fontsize=wb_font_sizes['s'], fontweight='normal', color='#666666',
                               ha='left', va='bottom')
        fig.canvas.draw()
...
def compute_total_bottom_margin(fig, axs, handles, note, note_margin_frac, spacing):
    ...
    xlabel_spacing = px_to_fig_frac(spacing["xl"], fig, axis='y')*4 if has_xlabel else px_to_fig_frac(spacing["xl"], fig, axis='y')*2
    legend_spacing_frac = px_to_fig_frac(spacing["xl"], fig, axis='y')

    if handles and not note:
        return legend_spacing_frac + px_to_fig_frac(spacing["xl"] * 4, fig, axis='y') + xlabel_spacing
    elif handles and note:
        return note_margin_frac + legend_spacing_frac + px_to_fig_frac(spacing["xl"] * 4, fig, axis='y') + xlabel_spacing
    elif note:
